...
### User story I.: Visit site, login, navigate to `/posts`
...
4. `/api/account/` gets the logged-in user's profile information.
...
6. The HTML is rendered for the Home Page

7. The rendered HTML generated by Vuex is sent back to the browser.

8. The user navigates (via a `<router-link>`/`<nuxt-link>`) to the login page. Since the navigation happened through Vue router, no request to the Nuxt server is made.
