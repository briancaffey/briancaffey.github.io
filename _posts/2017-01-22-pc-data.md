...
For the most part, the data I wanted to collect well organized and conveniently structured. Since the content on almost all of the pages is loaded dynamically, I used a JavaScript engine called PhantomJS to retrieve the HTML after the page loaded, otherwise scrapping the web pages would return only a skeleton of the DOM with no data. Here is the JavaScript file that I used with PhantomJS to scrape data: 
...
The first step is to loop through pages containing links of pages for both completed computer builds and computer parts using bash. Here's bash script I used to scrape links to computer builds:
...
```text
#http://stackoverflow.com/questions/28956622/how-to-locate-the-median-in-a-seaborn-kde-plot
#https://github.com/mwaskom/seaborn/issues/796
```

We can now revisit data from the collection of PC builds. First, we will take the price data and use it to fill missing values. To do this, we will be merging the part data frames with the PC builds data frame. Here's a bried description of database-style DataFrame joining/merging from the [pandas documentation](http://pandas.pydata.org/pandas-docs/stable/merging.html):
...
This will allow us to add additional information (as new columns) for each of the parts in the builds DataFrame. For example, we can add a column indicating the number of watts for the power supply of each build. Here's how we would perform this operation in pandas: 
